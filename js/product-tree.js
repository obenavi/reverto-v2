  ...
  modal.innerHTML = `
    ...
    <div style="padding:20px;flex:1;max-width:600px;width:100%;margin:0 auto">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px">
        ...
        <div>
          <label class="field-label">3PD% *</label>
          <input class="input" id="rb-olt" type="number" min="0" max="50" value="${recipe?.olt_commission_pct || 0}" oninput="rbRecalc()">
        </div>
      </div>

      <div style="font-size:11px;color:var(--on-surface-3);margin-bottom:14px">* 3PD = Third Party Delivery — Wolt / תן ביס / Uber Eats / DoorDash (עמלת הפלטפורמה על ההזמנה)</div>
